package cn.lili.modules.payment.kit.plugin.trc;
...
@Slf4j
@Component
public class TrcPlugin implements Payment {
    /**
     * 支付日志
     */
    @Autowired
    ...
    public ResultMessage<Object> nativePay(HttpServletRequest request, PayParam payParam) {
        ...
        // 查找在最近半小时内价格相同的代币
        List<Coin> coins = coinService.findCoinsWithSamePriceInTimeRange(CoinTypeEnum.TRC.name(), now.minusMinutes(30), now, cashierParam.getPrice());
        if (!coins.isEmpty()) {
            log.error("金额订单已存在");
            throw new ServiceException(ResultCode.PAY_ERROR);
        }
        TrcPaymentSetting trcpayPaymentSetting = alipayPaymentSetting();
        // 如果不存在，则创建新记录
        Coin newCoin = new Coin();
        newCoin.setCoinType(CoinTypeEnum.TRC.name());
        newCoin.setCoinAddress(trcpayPaymentSetting.getAggregateAddress()); // 这里需要设置实际的地址
        ...
        coins.add(newCoin);
        return ResultUtil.data(trcpayPaymentSetting.getAggregateAddress());
    }
